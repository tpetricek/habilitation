...
{\huge Charles University\\[0.2em]
Faculty of Mathematics and Physics}

\vfill

{\fontsize{30}{30}\selectfont Habilitation Thesis}

\vfill

{\huge 2024\hfill Tomas Petricek}
...
\noindent\includegraphics[width=40em]{img/mff.png}}}

\vspace{8em}

{\fontsize{30}{30}\selectfont Simple programming tools \\[0.2em] for data exploration}
...
{\Large\scshape Habilitation Thesis}

\vspace{1em}

{\Large Computer Science, Software Systems}

\vfill

{\Large Prague \hfill 2024}
...
end of my PhD at the University of Cambridge and finishing as I was joining the Department of
...
became my long-term mentor and friend. The work related to F\# received a warm welcome from the
friendly F\# community and early commercial adopters. Gustavo Guerra deserves special credit
...
first at the University of Cambridge and then at the University of Kent,
...
I had the pleasure of presenting many of the ideas at multiple industry conferences,
...
programming tools for data journalism for one and a half years. Microsoft Research,
BlueMountain Capital, the University of Kent, and Charles University paid for some of my time over
...
in Figure~\ref{fig:phelps}, which puts Phelps on a chart showing countries with similar numbers
of medals. Even such a simple visualization raises multiple questions. Is the table counting
...
for a non-programmer. The author has to acquire and clean the source data, aggregate medals by
...
follow-up questions. Data journalists sometimes offer a download of the original dataset, but the
...
to change the parameters of the visualization.
...
to easily make small changes, such as picking a different athlete or increasing the number of displayed
...
The criteria point to the technology gap illustrated by Figure~\ref{fig:gap} and there are multiple
...
notebook environment such as RStudio or Jupyter \citep{kluyver-2016-jupyter}, which make
...
programmatically, but even those have to be complemented with ad-hoc single-purpose tools,
...
The above holds for simple data explorations, such as those done by data journalists
...
programming language that implements it can be used to write programs that exhibit a certain kind
...
visualization language in Chapter~\ref{ch:galois}, to formalize the optimization technique introduced
...
presents the architecture and implementation of a novel computational notebook system for data
...
\item Finally, I also use the term simple when talking about programming systems and libraries
  ...
  Using such high-level abstractions means that programmers have to write less code.
\end{nitemize}
\vspace{0.5em}
...
a variant of the lifecycle that distinguishes between the exploration and production phases as done
...
for data access, but for the construction of SQL-like queries over tabular data. The research
...
twofold. First, it builds a practical efficient algorithm for displaying live previews. Second, it
...
tool. Using iterative prompting, the user can correct mistakes and configure the parameters of the tool.
...
and shapes) using a small number of combinators (such as overlaying and nesting of scales).
...
dynamic dependency program analysis, which is formalized and shown to have a desirable formal
...
Smalltalk. They also often interleave coding with execution, which is how most modern programs
...
ignores such stateful aspects of programming systems, possibly due to the paradigm shift documented
by \cite{rpg-2012-revolution}. This ignores an important aspect of the reality of modern programming.
...
retrieving data from a database. The aim of the work on type providers, outlined in this chapter,
...
Olympic medal history, gets data for Rio 2016 games, groups the data by the athlete, and sums their
...
operations and applies them to compatible data that exist in the data source. As I will show later,
...
compile-time. A type provider can run arbitrary code, such as accessing a database schema or
another external data source. It then generates a representation of a type that is
...
\item \emph{Real-world schema inference is hard.} Here, the response is an array always containing
  ...
  have a consistent structure, although some values may be \kvd{null}.
...
\item \emph{Safety guaranteed by static checks is relative.} Static type checking guarantees that
  ...
  runtime has a different structure, this will not prevent errors. We thus need to specify what
...
all of these three challenges. They can infer the shape of real-world data, infer types with a stable
...
can be analyzed at compile time (such as the \texttt{"worldbank.json"} file name above). It
...
value. The former consists of primitive shapes (inferred from a corresponding value) and
a record shape. The record shape has an (optional) name $\nu$ and consists of multiple fields
...
The above definition does not include the handling of choice shapes (corresponding to sum types), which
is introduced later.
...
returns a shape the most specific shape that can be used to represent the values of both of the two
...
shape of data. A data value is modeled formally as a value that can be either a primitive value
...
specified values. (Note that this can always be defined. In cases where values are of incompatible
...
properties of type providers, we do not need to elaborate on what an F\# type is here, but the most
...
write using a type provider (without using low-level functions that are only accessible
...
data source they use produces an input of a structure not encountered before. A typical example
...
shapes are sufficiently similar to be unified into a single type.) Using the formal model, we
...
\item \emph{Discoverability of members.} All querying logic is expressed through member accesses.
  ...
  and they can thus construct a query simply by repeatedly choosing one of the offered members.
...
of the given schema. The types of members of the class are further classes that allow further
querying. As the provided class structure is potentially infinite, it needs to be generated
lazily. The structure of the provided class definition, written as $L$ is thus a function mapping
...
in Chapter~\ref{ch:dotdriven} and is loosely modeled after standard object calculi
...
value of type \ident{Query}. Expressions of this type are those of relational algebra
...
behavior that one may otherwise encounter when accidentally accessing a non-existent field. This is
...
correct type. Although the property is not labeled as \emph{relative type safety} as in the
...
programs will ``not go wrong'' as long as the input has the correct structure (in a precisely defined
sense).
...
%
With slight notational creativity, we can say that the static type checking of a program that
...
programming environment tries to fill a particular space within the gap. It lets a relatively
...
approaches, and reverts some of their earlier experiments, they should always be able to know what
exact steps led to the final result they see on their screen.

In this chapter, I provide an overview of two contributions to the infrastructure for doing data
...
\item \emph{Polyglot tooling support.} Data scientist need an easy way of integrating multiple
  different tools. For example, they should be able to use simple data acquisition tools,
...
address all of them. This chapter provides an overview of work leading towards such a system.
...
on live previews for data exploration environments presents a formal model to explain how
...
\item \emph{Responding to code changes.} Code in a text editor can change in arbitrary ways and
  so it is unclear how to update the existing live preview when an edit is made. This is easier in
...
abstract syntax tree. The nodes of the tree are then analyzed and linked to nodes in a
dependency graph. When the node of the tree corresponds to a dependency graph node that has been
...
at the top-level and expressions that can appear as arguments in an invocation.
...
Compositionality implies the deterministic behavior of external libraries and is essential for
...
full in Chapter~\ref{ch:foundations}) uses the calculus to formalize an efficient mechanism
for showing live previews during the editing of data exploration script. As mentioned earlier,
...
differ in their dependencies. The graph edges are labeled with labels indicating the kind of
...
different dependencies will contain a different symbol. Dependencies of a member access are labeled
...
The construction of the graph makes it possible to compute previews over the nodes of the
...
way of obtaining a suitable value for the variable. In this case, our mechanism evaluates
...
and they also have an interesting theoretical connection to work on Contextual Modal Type Theory
...
model based on the data exploration calculus as sketched above. The formalization is
...
Chapter~\ref{ch:wrattler}. Wrattler is an extension of the industry standard JupyterLab platform.
...
As a result, it is possible to integrate tools that provide a live preview mechanism such as
...
\begin{nitemize}
\item \emph{Polyglot architecture.} The system is designed to allow the integration of components in
...
\end{tabular}
\caption{In notebook systems such as Jupyter, state and execution are managed by a kernel. In
...
The notebook runs in a browser and sends commands to the kernel in order to evaluate
...
individual Wrattler components and provides persistent data storage.
...
data frames. The R and Python cells are independent and map to independent graph nodes. The node
...
Section~\ref{sec:infra-livedata}. As before, the dependency graph is used to cache the results
...
In the case of Python, R, and JavaScript, this is done using a lightweight code analysis.
In the case of The Gamma, which can also be used in Wrattler, the full
parse tree and its associated dependency graph are available. A prototype extension of Wrattler
...
source code or variable name) and the hashes of nodes it depends on. An important property of this
...
\emph{data exploration calculus}, they present the architecture for \emph{polyglot, live and reproducible
...
characterization of a clean dataset that tools could optimize for. Human insight into the data
is always needed.
...
The contributions outlined in this chapter are centered around the question of how to
easily enable human data analysts, even if they are not expert programmers, to supply the
necessary human insight to programmatic tools when cleaning and analyzing data.
...
The primary contribution of the work presented in this chapter is that it develops and validates
...
with a small number (typically 1-2) of simple interactions.
...
problem of data wrangling and data exploration. But in order to fulfill this role, they need to
...
\citep{syme-2013-inforich} and type providers (Chapter~\ref{ch:tp}). It is centered around
...
\noindent
The query is illustrated in Figure~\ref{fig:graphtp}. We start from the \texttt{Doctor} node
...
relationships. The result returned by the provider is a table of properties of all nodes
...
allow selecting a subset of the data, but it can be used to construct SQL-like queries.
...
\item \emph{How do users understand complex query languages?}
...
operations, modeled as mathematical functions between different sets.
...
AI-based data wrangling tools that follow the \emph{onetime interaction} pattern and turn them
...
tools, as well as multiple \emph{type providers} for accessing data in graph databases,
data cubes, and relational databases.}

\noindent
This chapter brings together two contributions that aim to reduce the gap between programming
...
characteristics of the type provider that make this possible are that it is complete and correct,
...
often analyze data in order to find interesting insights and share those with their readers.
...
make creating such a chart a challenging programming problem:

\begin{nitemize}
\item The chart combines multiple different visual elements. In addition to the bars themselves,
  it also needs to include the markers (dashed lines) that show the guess.
\item The chart uses a custom color scheme, and background to indicate possible areas of the bar and it
...
visualizations based on the code of scripts that construct charts from shared data.
The two systems are aimed at programmers, but they are simple in that they make it possible
to create sophisticated interactive visualizations using a small amount of straightforward code.
...
Higher-level compositional approaches to chart construction are typically based on the grammar of
graphics \citep{wilkinson-1999-grammar}. In the grammar of graphics, a chart is a mapping from
...
\section{Composable data visualisations}
...
compares UK election results for the years 2017 and 2019. In the chart, the $x$ axis shows categorical
...
colours for 2017 and 2019 and the number of MPs for 2017 and 2019, we can construct the chart in
...
representing the number of MPs for 2017 and 2019, respectively. Each rectangle is specified
...
a given shape (specified in pixels). We generate a pair of rectangles for each party using a
...
\item \emph{Inferring the scales of a shape}. The implementation first recursively walks over
  ...
  shapes. In the case of continuous scale, we take the overall minimum and maximum. For a categorical
...
\item \emph{Projecting coordinates}. Once the chart is annotated with the inferred scales, we
  ...
  (in pixels) and a value on the scale. The results are $x$ and $y$ coordinates in pixels.
...
process is conceptually simple, there are a number of subtle details. In particular,
...
noting that some operations, such as \texttt{axis} that add an axis with labels, can be eliminated
...
a chart based on the current state. They also specify a function that updates the state when
...
When the user clicks on a bar, the event handler receives a pair of values such as
...
graphical elements. This is necessary if we want to create rich interactive charts. However a single
...
2015 and the ratio of energy produced in the USA and China over time for each type of energy.
...
traditionally been used in areas such as information-flow security, optimization, and
...
approaches based on the grammar of graphics, we are not restricted to a set of pre-defined primitives.
...
a given country and energy type, \texttt{plot} helper that calculates the ratio between the USA and
...
resources (code and data) needed to produce the entire result, but only in the resources needed to
...
value. (We use this mechanism for analyzing charts, but it could equally be used to analyze code
...
the following judgment states that the program reduces to a value $v$, producing a trace $T$:
%
\begin{equation*}
...
step and displayed using different colors).
...
data in multiple charts. The system works by analyzing the program that is used to construct the
...
\iftoggle{fulltext}{\includepdf[pages=-]{papers/fsdata.pdf}}

\chapter{Data exploration through dot-driven development}
\label{ch:dotdriven}

\bibentry{petricek-2017-dot}
\iftoggle{fulltext}{\includepdf[pages=-]{papers/dotdriven.pdf}}
...
\iftoggle{fulltext}{\includepdf[pages=1-36]{papers/foundations.pdf}}
...
\iftoggle{fulltext}{\includepdf[pages=-]{papers/wrattler.pdf}}
...
\iftoggle{fulltext}{\includepdf[pages=-]{papers/thegamma.pdf}}

\chapter{AI Assistants: A framework for semi-automated data wrangling}
\label{ch:aia}

\bibentry{petricek-2023-aia}
\iftoggle{fulltext}{\includepdf[pages=-]{papers/aia.pdf}}
...
\iftoggle{fulltext}{\includepdf[pages=-]{papers/compost.pdf}}

\chapter{Linked visualizations via Galois dependencies}
\label{ch:galois}

\bibentry{petricek-2022-galois}
\iftoggle{fulltext}{\includepdf[pages=-]{papers/galois.pdf}}
...
\chapter{Contributions and outlook}
%\label{ch:contribs}
%\addcontentsline{toc}{chapter}{\nameref{ch:contribs}}

The work presented in this thesis is the result of my long-term effort to
rethink data science tooling from the perspective of programming languages research.
The research has been undertaken at multiple institutions (Microsoft Research, The Alan
...
and libraries. The resulting software packages have been made available as open-source.
Some were developed by a larger team of collaborators, while others later received valuable
contributions from the broader community. In some cases, industry adopters further developed
the project and became maintainers of the tool.

This chapter briefly reviews my own contributions to the papers included in this thesis,
as well as my role in the resulting open-source projects. I will then reflect on the new
perspective on programming and data science tooling that emerges from the work, as well as
future research directions inspired by the presented work.
...
with the work of my colleagues and collaborators.
...
\item \emph{Chapter~\ref{ch:fsdata}} \citep{petricek-2016-data} -- I developed the initial version
  ...
  with the formalization, and provided the problem framing.
...
\begin{nitemize}
  ...
  \item \emph{Fluid} (\url{https://github.com/explorable-viz/fluid}) is a programming language
    ...
    Achintya Rao. I participated in the project in an advisory role and collaborated on some of the
...
The first unifying theme is the broader motivation. If society is to benefit from the
increasing availability of open data and data processing capabilities, we must make working
with data accessible to a broader audience. Experts who are not trained as programmers need to
...
it possible to restrict problems in ways that would, in other contexts, seem too constrained.
...
how we conceptualize programming languages. We need to think less about \emph{programming languages}
...
the current state, scripts that data scientists are tweaking and what they see on the screen,
