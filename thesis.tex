...
The logic of working with AI assistants is illustrated in Figure~\ref{fig:interaction}. When using
the assistant, we start with the empty interaction $H_0$. We then iterate until the human analyst
accepts a proposed data transformation. In each iteration, we first invoke $\best_\Din(H)$ to get
...
option $H_i$ (where $i\in \{1,\ldots,n\}$). We use this choice as a new human interaction $H$ and call
